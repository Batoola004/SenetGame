...
// //كلاس واجهات
// //تحتوي على منطق التحريك في الواجهة ايضا وفيها توليد البورد الاولي
// import javax.swing.*;
...
//     // تابع رسم شكل بورد اللعبة في الواجهة
...
public class SenetGame {
    ...
    private boolean gameActive = true;
    private boolean aiThinking = false;
    private boolean canHumanRoll = true; // متغير لمنع الرمي المتعدد في نفس الدور

    // حجم لوحة اللعبة (30 مربع)
    private static final int BOARD_SIZE = 30;

    public SenetGame() {
        initializeBoard(); // إعداد اللوحة
        initializeUI(); // بناء الواجهة
        startGame(); // بدء اللعبة
    }

    // إعداد الحالة الابتدائية للوحة
    private void initializeBoard() {
        board = new int[BOARD_SIZE];
        for (int i = 0; i < 14; i++) {
            board[i] = (i % 2 == 0) ? Board.HUMAN : Board.AI;
    ...
    private void initializeUI() {
        ...
        // مؤقت لدور الذكاء الاصطناعي (يشتغل بعد ثانية واحدة)
        aiTimer = new Timer(1000, e -> {
            aiThinking = false;
            if (gameActive && currentPlayer == Board.AI) {
                executeAITurn();
    ...
    // بدء اللعبة وإعداد الحالة الابتدائية
    private void startGame() {
        updateBoardDisplay();
        updateInfoPanel();
        controlPanel.setGameControlsEnabled(true);
    ...
    // تحديث عرض اللوحة على الشاشة
    public void updateBoardDisplay() {
        boardPanel.setBoard(board);
        boardPanel.repaint();
    }

    // تحديث لوحة المعلومات (دور اللاعب والقطع الخارجة)
    private void updateInfoPanel() {
        infoPanel.updateInfo(currentPlayer, humanExited, aiExited);
    }

    // معالجة رمي العصي من قبل الإنسان
    public void rollDiceForHuman() {
        // التحقق من جميع الشروط قبل السماح بالرمي
        if (!gameActive) {
        ...
        // منع الرمي المتعدد في نفس الدور
        canHumanRoll = false;
    ...
    // ------ دوال مساعدة للمنطق الأساسي ------

    // تطبيق المنطق الأساسي للحركة
    private void performMoveLogic(Move move, int player) {
        boardPanel.highlightMove(move);
        Board.applyMove(board, move, player);
    ...
    // معالجة القطع الموجودة في المربعات الخاصة (28، 29، 30) التي لم تلعب
    private void handleUnplayedSpecialPieces(int player, Move moveJustMade) {
        int[] specialSquares = { Rules.HOUSE_OF_THREE_TRUTHS, Rules.HOUSE_OF_RE_ATUM, Rules.HOUSE_OF_HORUS };
    ...
    // إرسال قطعة إلى بيت الولادة (15) أو أقرب مربع فارغ قبله
    private void sendPieceToRebirthOrBefore(int currentSquare, int player) {
        int targetIdx = Rules.HOUSE_OF_REBIRTH - 1;
    ...
    // تبديل الدور بين الإنسان والذكاء الاصطناعي
    private void switchTurn() {
        if (!gameActive)
            return;
        ...
        infoPanel.showMessage("Switching turn...");
        currentPlayer = -currentPlayer;
        canHumanRoll = (currentPlayer == Board.HUMAN); // إعادة تعيين حالة الرمي
    ...
    // التحقق من نهاية اللعبة (فوز أحد اللاعبين)
    private void checkGameEnd() {
        if (humanExited >= TOTAL_PIECES) {
            infoPanel.showMessage("HUMAN WINS! All pieces exited!");
            gameActive = false;
    ...
    // إعادة تعيين اللعبة إلى الحالة الابتدائية
    public void resetGame() {
        if (aiTimer != null && aiTimer.isRunning()) {
            aiTimer.stop();
        }
    ...
    // نقطة الدخول الرئيسية لتشغيل اللعبة
    public static void launchGame() {
        SwingUtilities.invokeLater(() -> {
            try {
                UIManager.setLookAndFeel(UIManager.getSystemLookAndFeelClassName());
